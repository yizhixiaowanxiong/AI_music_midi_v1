...
#
# from renderer import render_arrangement_to_midi
#
# # === 乐器映射协议 ===
...
def parse_arrangement(payload: Any) -> ArrangementSchema:
    """
    payload 支持：
    - ArrangementSchema 实例
    - JSON 字符串（LLM 常见返回）
    - dict（已 json.loads 的对象）
    """
    if isinstance(payload, ArrangementSchema):
        return payload

    # JSON 字符串：用 pydantic v2 的 model_validate_json（推荐）:contentReference[oaicite:1]{index=1}
    if isinstance(payload, str):
        return ArrangementSchema.model_validate_json(payload)

    # dict / 其它对象：用 model_validate
    return ArrangementSchema.model_validate(payload)


def export_midi(
    payload: Any,
    out_path: Union[str, Path] = "outputs/song.mid",
    *,
    ppq: int = DEFAULT_PPQ,
    ...
    try:
        arrangement = parse_arrangement(payload)
    except ValidationError as e:
        # 把错误抛给上层，agent 可以打印 e.errors() 做定位
        raise

    # 交给 renderer：renderer 内部负责 tempo/time_signature/track 写入等 MIDI 细节
    # Mido 的 MIDI 文件里 tempo/time_signature 都是 MetaMessage 写入（renderer 会做）:contentReference[oaicite:2]{index=2}
    midi_path_str = render_arrangement_to_midi(
        arrangement,
...
def export_midi_from_llm_json(
    llm_json_str: str,
    *,
    out_dir: Union[str, Path] = DEFAULT_OUT_DIR,
    filename: str = "song.mid",
    ppq: int = DEFAULT_PPQ,
    seed: Optional[int] = 1234,
) -> Path:
